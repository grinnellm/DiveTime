...
# "15:12" for each diver, for example "MG.Start", and "MG.End". See the example 
...
CalcRawTime <- function( dat ) {
  ...
  if( any(timeMat$Time <= 0) )  stop( "Non-positive dive time(s)", call.=FALSE )
...
rawMinsTot <- bind_rows( rawMinsWide, 
    data.frame(Date="Total", Transect="Total", 
        t(colSums(select(rawMinsWide, -Date, -Transect)))) )
...
rawMinsDayTot <- bind_rows( rawMinsDayWide, 
    data.frame(Date="Total", t(colSums(select(rawMinsDayWide, -Date)))) )
...
adjHrsDayTot <- bind_rows( adjHrsDayWide, 
    data.frame(Date="Total", t(colSums(select(adjHrsDayWide, -Date)))) )
